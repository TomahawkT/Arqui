...
int main(void){
    ...
    sys_read(STDIN,input,BUFFER_MAX);
    int fd = sys_open("Datos.txt",_O_RDONLY,S_IRUSR);
    if (fd<0){
    ...
    while (token != NULL){
        printf("%s\n", token);
        ocurrences += strcasecmp(token,input) == 0;
        token = strtok(NULL," ");
    }
    char resp[BUFFER_MAX] = {0};
    sprintf(resp,"Ocurrencias: %d\n",ocurrences);
    sys_write(STDOUT,resp,strlen(resp));
    return 0;
}